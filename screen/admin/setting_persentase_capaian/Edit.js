...
const EditPersentaseCapaian = ({navigation, route}) => {
  ...
  const fetchEditData = async uuid => {
    try {
      const response = await apiClient.get(
        `/setting_persentase_capaian/${uuid}/edit`,
      );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.cardContainer}>
        ...
        <Dropdown
          ...
          placeholder="Jabatan"
          placeholderStyle={{color: '#B6B9CA'}}
          value={editData.name} // Menggunakan `jabatan_id` sebagai value
          onChange={
            item => setEditData(prev => ({...prev, name: item.value})) // Perbarui `jabatan_id` sesuai pilihan
          }
          renderItem={item => (
            <Text
              style={[styles.dropdownItem, styles.customFont, {color: '#333'}]}>
        ...
        />
        <Text style={styles.label}>Max Persentase Capaian</Text>
        <TextInput
          style={styles.input}
...
export default EditPersentaseCapaian;
