...
const TambahUserGroup = ({navigation}) => {
  const [selectedNamaGroup, setSelectedNamaGroup] = useState('');
  ...
  const submitTambah = async () => {
    setIsLoading(true);
    try {
      await apiClient.post('/user_group/create', {
        nm_user_group: selectedNamaGroup,
      });
  ...
  return (
    <View style={styles.rootContainer}>
      <Header title="Tambah User Group" />
          ...
          <ScrollView
            ...
            <TextInput
              style={[
                ...
                styles.input,
                focusState.selectedNamaGroup && styles.inputFocused,
                selectedNamaGroup && styles.inputFilled,
              ]}
              placeholder="Nama User Group"
              multiline
              value={selectedNamaGroup}
              onChangeText={setSelectedNamaGroup}
              placeholderTextColor="#B0B0B0"
              onFocus={() => handleFocus('selectedNamaGroup')}
              onBlur={() => handleBlur('selectedNamaGroup')}
            />
...
export default TambahUserGroup;
