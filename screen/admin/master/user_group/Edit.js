...
const EditUserGroup = ({navigation, route}) => {
  ...
  const submitEdit = async () => {
    setIsLoading(true);
    try {
      await apiClient.post(`/user_group/${editData.uuid}/update`, {
        nm_user_group: editData.nm_user_group,
      });
      navigation.goBack();
  ...
  const fetchEditData = async uuid => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/user_group/${uuid}/edit`);
      ...
      <View style={styles.container}>
        {isLoading ? (
          ...
          <ScrollView
            ...
            <Text style={[GlobalStyle.SemiBold, styles.label]}>Nama User Group</Text>
            <TextInput
              style={[
                GlobalStyle.SemiBold,
                styles.input,
                focusState.nm_satuan && styles.inputFocused,
                editData.nm_satuan && styles.inputFilled,
              ]}
              placeholder="Nama User Group"
              value={editData.nm_user_group || ''} // Pastikan menggunakan default kosong jika null
              onChangeText={text =>
                setEditData(prev => ({...prev, nm_user_group: text}))
              }
              placeholderTextColor="#B0B0B0"
              onFocus={() => handleFocus('nm_satuan')}
              onBlur={() => handleBlur('nm_satuan')}
            />
