...
const EditPajakPtkp = ({navigation, route}) => {
  ...
  const submitEdit = async () => {
    setIsLoading(true);
    try {
      await apiClient.post(`/pajak_ptkp/${editData.uuid}/update`, {
        ptkp: editData.ptkp,
  ...
  const fetchEditData = async uuid => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/pajak_ptkp/${uuid}/edit`);
          ...
          <ScrollView
            ...
            <TextInput
              style={[
                ...
                styles.input,
                focusState.ptkp && styles.inputFocused,
                editData.ptkp && styles.inputFilled,
              ]}
              ...
              onFocus={() => handleFocus('ptkp')}
              onBlur={() => handleBlur('ptkp')}
            />
            ...
            <TextInput
              style={[
                GlobalStyle.SemiBold,
                styles.input,
                focusState.pajak && styles.inputFocused,
                editData.pajak !== '' &&
                editData.pajak !== null &&
                editData.pajak !== undefined
                  ? styles.inputFilled
                  : null,
              ]}
              placeholder="Nominal"
