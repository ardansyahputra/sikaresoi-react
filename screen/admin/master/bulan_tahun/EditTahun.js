...
const EditTahun = ({navigation, route}) => {
  ...
  const submitEdit = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.post(`/tahun/${editData.uuid}/update`, {
  ...
  const fetchEditData = async uuid => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/tahun/${uuid}/edit`);
          ...
          <ScrollView
            ...
            <TextInput
              style={[
                ...
                styles.input,
                focusState.tahun && styles.inputFocused,
                editData.tahun && styles.inputFilled,
              ]}
              placeholder="Tahun"
              value={editData.tahun || ''}
              onChangeText={text =>
                setEditData(prev => ({...prev, tahun: text}))
              }
              placeholderTextColor="#B0B0B0"
              onFocus={() => handleFocus('tahun')}
              onBlur={() => handleBlur('tahun')}
            />
