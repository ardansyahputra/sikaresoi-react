...
export default function Kegiatan() {
  ...
  const fetchUraianShow = async () => {
    try {
      const response = await axios.get(
        'http://192.168.60.123:8000/api/v1/uraian/show',
        {
          headers: {
            Authorization: token,
          },
  ...
  const fetchData = async page => {
    try {
      ...
      const response = await axios.post(
        'http://192.168.60.123:8000/api/v1/kegiatan/index',
        {page},
        {
          headers: {
            Authorization: token,
  ...
  const submitEdit = async () => {
    try {
      await axios.post(
        `http://192.168.60.123:8000/api/v1/kegiatan/${editData.uuid}/update`,
        {
          nm_kegiatan: editData.nm_kegiatan,
          point: editData.point,
          uraian_id: pickUraianOptions || null,
        },
        {
          headers: {
            Authorization: token,
      ...
      if (error.response) {
        ...
        Alert.alert(
          'Error',
          'Gagal memperbarui data. Tidak ada respons dari server. Periksa koneksi jaringan Anda.',
        );
      } else {
  ...
  const fetchEditData = async uuid => {
    try {
      const response = await axios.get(
        `http://192.168.60.123:8000/api/v1/kegiatan/${uuid}/edit`,
        {
          headers: {
            Authorization: token,
          },
  ...
  const submitHapus = async () => {
    try {
      await axios.delete(
        `http://192.168.60.123:8000/api/v1/kegiatan/${selectedUuid}/delete`,
        {
          headers: {
            Authorization: token,
          },
  ...
  const submitTambah = async () => {
    try {
      await axios.post(
        'http://192.168.60.123:8000/api/v1/kegiatan/create',
        {
  ...
  const handleCloseTambahModal = () => {
    setSelectedNamaKegiatan('');
    setSelectedPoint('');
    setPickUraianOptions(null);
    setTambahModalVisible(false);
  };
  ...
  const TableHeader = () => (
    <View>
      ...
      <View style={styles.tableHeader}>
        ...
        <Text style={[styles.headerCell, styles.nameCell]}>Nama Kegiatan</Text>
        <Text style={[styles.headerCell, styles.tableStatusCell]}>Point</Text>
      ...
      <Modal
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalLabel}>Nama Kegiatan</Text>
            <TextInput
            ...
            />
            <Text style={styles.modalLabel}>Point</Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Point"
              value={
              ...
              placeholderTextColor={'#B6B9CA'}
            />
            <Text style={styles.modalLabel}>Uraian</Text>
            <Dropdown
              style={styles.modalInput}
              data={uraianOptions}
              labelField="label"
