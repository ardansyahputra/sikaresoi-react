...
export default function Reward({route, navigation}) {
  ...
  useEffect(() => {
    ...
    if (!reward_punishment_id) {
      console.error('reward_punishment_id is undefined');
      Alert.alert('Error', 'ID tidak valid.');
      navigation.goBack();
    } else {
      // Fetch data sesuai tombol yang aktif
      if (activeButton === 'reward') {
        fetchRewardData(currentPage);
      } else {
        fetchPunishmentData(currentPage);
      }
    }
  }, [activeButton, reward_punishment_id, currentPage]);
  ...
  const fetchRewardData = async () => {
    try {
      ...
      if (!reward_punishment_id) {
        ...
        Alert.alert('Error', 'reward_punishment_id tidak boleh kosong.');
        setLoading(false);
        return;
      }

      const response = await apiClient.post(
        `reward/index?reward_punishment_id=${reward_punishment_id}`,
      );
  ...
  const fetchPunishmentData = async () => {
    try {
      ...
      const response = await apiClient.post(
        `punishment/index?reward_punishment_id=${reward_punishment_id}`,
      );
      setData(response.data.data);
    } catch (error) {
      console.error('Error fetching reward data:', error.message);
  ...
  const submitHapus = async () => {
    try {
      const endpoint =
        activeButton === 'reward'
          ? `/reward/${selectedUuid}/delete`
          : `/punishment/${selectedUuid}/delete`;
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        fetchPunishmentData(currentPage);
        fetchPunishmentData(currentPage); // Refresh data
      } else {
  ...
  const handlePress = buttonName => {
    ...
    if (buttonName === 'reward') {
      fetchRewardData(1); // Langsung fetch data saat tombol Bulan dipilih
    } else {
      fetchPunishmentData(1); // Langsung fetch data saat tombol Tahun dipilih
    }
  };

  const headerText = activeButton === 'reward' ? 'Tanggal' : 'Tanggal';
  ...
  const renderItem = ({item, index}) => {
    const tahunBulanData =
      activeButton === 'reward' ? item.tanggal : item.tanggal;
          ...
          <View style={styles.bulanContainer}>
            <Pressable
              style={({pressed}) => [
                ...
                activeButton === 'reward' && styles.buttonActive,
              ]}
              ...
              <View style={styles.bulanButton}>
                <Text
                  style={[
                    styles.buttonText,
                    activeButton === 'reward' && styles.textActive,
                  ]}>
                  Reward
                </Text>
              </View>
