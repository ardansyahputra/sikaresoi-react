...
const EditReward = ({navigation, route}) => {
  ...
  const submitEdit = async () => {
    setIsLoading(true);
    try {
      await apiClient.post(`/reward/${editData.uuid}/update`, {
        tanggal: Tanggal,
  ...
  const fetchEditData = async uuid => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/reward/${uuid}/edit`);
      ...
      <View style={styles.container}>
        {isLoading ? (
          ...
          <ScrollView
            ...
            <Text style={[GlobalStyle.SemiBold, styles.label]}>Reward</Text>
            <TextInput
              style={[
                GlobalStyle.SemiBold,
                styles.input,
                focusState.reward && styles.inputFocused,
                editData.reward && styles.inputFilled,
              ]}
              placeholder="Reward"
              value={editData.reward || ''}
              onChangeText={text =>
                setEditData(prev => ({...prev, reward: text}))
              }
              placeholderTextColor="#B0B0B0"
              onFocus={() => handleFocus('reward')}
