...
const ProfileScreen = () => {
      ...
      apiClient
        ...
        .then(response => {
          ...
          if (Array.isArray(pangkatList)) {
            const foundPangkat = pangkatList.find(
              item => item.id === user.pangkat_id,
            );

            if (foundPangkat) {
              const {nm_pangkat, golongan, ruang} = foundPangkat;
              // Format: Nama Pangkat (Golongan/Ruang)
              const formattedPangkat = `${nm_pangkat} (${golongan}/${
                ruang || '-'
              })`;
              setPangkat(formattedPangkat);
            } else {
              setPangkat('Pangkat Tidak Ditemukan');
            }
          } else {
  ...
  const handleChangePhoto = imageUri => {
    if (imageUri) {
      setProfileImage({uri: imageUri});
      Toast.show({
        type: 'success',
        text1: 'Foto Profil Berhasil Diubah',
      });
    } else {
      Toast.show({
        type: 'info',
        text1: 'Pilih Foto Dibatalkan',
      });
    }
  };
      ...
      >
        <View style={styles.profileHeader}>
          ...
          <Button
            title="UBAH FOTO"
            onPress={() => openGallery()}
            color="#4A90E2" // Bisa mengganti warna tombol sesuai dengan yang diinginkan
          />
          ...
          <Text style={styles.profileSubtitle}>{user?.nip || 'User Nip'}</Text>
        </View>
        ...
        <View style={styles.menuContainer}>
          ...
          <TouchableOpacity
            style={styles.menuOption}
            onPress={navigateToPasswordEdit}>
            <Ionicons
              name="lock-closed"
              size={20}
              color="#1D56C0"
